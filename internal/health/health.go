...
// FetchStartedAt reads the "started_at" field from an HTTP health endpoint.
// Returns zero time if the field is absent or the request fails.
func FetchStartedAt(port int, endpoint string) time.Time {
	if endpoint == "" {
		endpoint = "/health"
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil || payload.StartedAt == "" {
		return time.Time{}
	}
