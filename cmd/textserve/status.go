...
func printStatusTable(cmd *cobra.Command, rows []serverRow) {
	...
	for _, r := range rows {
		...
		regStr := formatReg(r.registered, r.noRegister)
...
func formatReg(registered, noRegister bool) string {
	if noRegister {
		return "-"
	}
	if registered {
...
func nativeUptime(cfg *registry.ServerConfig) string {
	// Prefer started_at from the health endpoint when the server exposes one.
	if cfg.Port > 0 {
		if t := health.FetchStartedAt(cfg.Port, cfg.Health.Endpoint); !t.IsZero() {
			return formatDuration(time.Since(t))
		}
	}
	if cfg.PidFile == "" {
