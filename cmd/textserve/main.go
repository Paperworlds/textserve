...
func configFilePath() string {
	return filepath.Join(os.Getenv("HOME"), ".local", "paperworlds", "textserve", "config.yaml")
}

// readConfigRoot reads the root field from ~/.local/paperworlds/textserve/config.yaml.
...
// then falls back to the root set in ~/.local/paperworlds/textserve/config.yaml.
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, n := range names {
				...
				claude.Deregister(n, cfg) //nolint:errcheck — always clean up stale entries
				switch cfg.Runtime {
				case registry.RuntimeProcess:
					_ = native.Stop(n, cfg)
				default:
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, n := range names {
				...
				cfg := serverConfig(repoRoot, n, entry)
				if cfg.NoRegister {
					fmt.Printf("%s has no_register: true — skipping\n", n)
					continue
				}
				resolvePreStart(repoRoot, cfg)
				if err := claude.Register(n, cfg); err != nil {
